...
def upload_skip():
    ...
    #         skip_sens_content.append(c.fetchone()[0].capitalize())
    global dur_item, files_path, sens_id_arr, skip_sens_id, skip_sens_content
    dur_item, files_path, sens_id_arr, skip_sens_id, skip_sens_content = get_each_info(id)
    print(skip_sens_id)
...
@app.route("/thongke_admin_user")
def thongke_admin_user():
    id = session['id']
    cur_id = session['cur_id']
    with sqlite3.connect("transcripts.db") as conn:
        c = conn.cursor()
    ...
    return render_template("thongke_admin_user.html", fullname = fullname, users_info = users_info, users_num = len(users_info))

@app.route("/thongke_admin_sentences")
def thongke_admin_sentences():
    id = session['id']
    cur_id = session['cur_id']
    with sqlite3.connect("transcripts.db") as conn:
        c = conn.cursor()
            ...
            if item[2]: # sentence_01 - tên của folder lưu các file ghi âm của từng câu
                ...
                if os.path.exists(folder_save):
                    ...
                    if len(files_arr):
                        ...
                        sentence.append(len(files_arr)) # số người dùng đã thu
                        sentences_info.append(sentence)
    return render_template("thongke_admin_sentences.html", fullname= fullname, sentences_info= sentences_info, sens_num = len(sentences_info))
